...
for tool in git uv gcc g++ cmake ninja rustc cargo; do
...
export MAX_JOBS=4 CMAKE_BUILD_PARALLEL_LEVEL=4 NVCC_THREADS=1 TORCH_CUDA_ARCH_LIST=12.1a
...
uv pip install --python "$setup_root/venv/bin/python" \
  -r requirements/build/cuda.txt -r requirements/build/rust.txt -r requirements/cuda.txt
uv pip install --python "$setup_root/venv/bin/python" --no-build-isolation -e .
