...
def test_mtp_quantization_maps_local_indices_without_guessing_other_layers():
    """A draft scale entry must follow its own layer, including multi-layer MTP."""
    from vllm.models.qwen4_exp.nvidia.mtp import _remap_mtp_quantized_layers

    first = {"quant_algo": "FP8_BLOCK_SCALES", "group_size": 128}
    second = {"quant_algo": "NVFP4", "group_size": 16}
    entries = {
        "mtp.layers.0.mlp.experts": first,
        "mtp.layers.1.mlp.experts": second,
        "model.layers.0.mlp.experts": second,
    }
    actual = _remap_mtp_quantized_layers(entries, 48, 2)
    assert actual == {
        "mtp.layers.48.mlp.experts": first,
        "mtp.layers.49.mlp.experts": second,
        "model.layers.0.mlp.experts": second,
    }
    assert "mtp.layers.0.mlp.experts" in entries
    assert _remap_mtp_quantized_layers(actual, 48, 2) == actual
    with pytest.raises(ValueError, match="Conflicting MTP"):
        _remap_mtp_quantized_layers(
            {**entries, "mtp.layers.48.mlp.experts": second}, 48, 2
        )
