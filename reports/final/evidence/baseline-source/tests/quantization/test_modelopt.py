...
def test_modelopt_mixed_precision_dispatches_block_fp8_expert_scales(monkeypatch):
    """NVIDIA MTP block scales must not fall through to unquantized experts."""
    from vllm.model_executor.layers.quantization import fp8
    from vllm.model_executor.layers.quantization.modelopt import RoutedExperts
    ...
    config = ModelOptMixedPrecisionConfig.from_config(
        {
            "quantization": {
                ...
                "exclude_modules": [],
                "quantized_layers": {
                    prefix: {"quant_algo": "FP8_BLOCK_SCALES", "group_size": 128}
                },
            }
        }
    )
    constructor = Mock(return_value=object())
    monkeypatch.setattr(fp8, "Fp8MoEMethod", constructor)
    layer = MagicMock(spec=RoutedExperts)
    assert config.get_quant_method(layer, prefix) is constructor.return_value
    block_config, passed_layer = constructor.call_args.args
    assert passed_layer is layer
