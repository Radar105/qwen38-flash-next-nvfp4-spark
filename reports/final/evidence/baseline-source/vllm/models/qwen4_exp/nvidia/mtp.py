...
def _remap_mtp_quantized_layers(
    layers: dict[str, Any], start: int, count: int
) -> dict[str, Any]:
    """Align draft-local ModelOpt indices with Qwen4Exp's global layer names."""
    remapped: dict[str, Any] = {}
    for name, info in layers.items():
        match = re.match(r"^(.*?mtp\.layers\.)(\d+)(\..*)$", name)
        if match and int(match[2]) < count:
            name = f"{match[1]}{start + int(match[2])}{match[3]}"
        if name in remapped and remapped[name] != info:
            raise ValueError(f"Conflicting MTP quantization entries for {name}")
        remapped[name] = info
    return remapped
...
def _make_draft_vllm_config(
    ...
    if draft_quant_config is not None:
        ...
        if isinstance(draft_quant_config, ModelOptMixedPrecisionConfig):
            draft_quant_config.quantized_layers = _remap_mtp_quantized_layers(
                draft_quant_config.quantized_layers,
                mtp_start_layer_idx,
                speculative_config.draft_model_config.hf_text_config.mtp_num_hidden_layers,
            )
