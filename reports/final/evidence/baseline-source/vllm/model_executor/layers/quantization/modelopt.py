    ...
    def get_quant_method(
        ...
        if isinstance(layer, RoutedExperts):
            ...
            if quant_algo == "FP8_BLOCK_SCALES":
                ...
                group_sizes = {
                    info.get("group_size", 128)
                    for candidate in self._quantized_layer_prefix_candidates(prefix)
                    for name, info in self.quantized_layers.items()
                    if name == candidate or name.startswith(candidate + ".")
                }
                if len(group_sizes) != 1:
                    raise ValueError(f"Ambiguous FP8 block size for {prefix}")
                group_size = group_sizes.pop()
                if not isinstance(group_size, int) or group_size <= 0:
                    raise ValueError(f"Invalid FP8 block size for {prefix}")
                return Fp8MoEMethod(
                    Fp8Config(
                        is_checkpoint_fp8_serialized=True,
